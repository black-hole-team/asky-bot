...
/**
 * Вспомогательный класс для {@link NoneHandler}
 */
@RequiredArgsConstructor(onConstructor_ = @__(@Inject))
public class NoneHandlerHelper {
            ...
            if (ticket.getKey()) {
                for (var hub : routeChannelHubs) {
                    ...
                    stageRepository.findByChannelTypeAndUserIdAndChatId(hub.getChannelId(), hub.getChannelHubId())
                            .ifPresent(stage -> {
                                if (stage.getData(NoneHandler.TICKET_ID_STATE_DATA).isEmpty()) {
                                    stage.setData(NoneHandler.TICKET_ID_STATE_DATA, String.valueOf(ticket.getValue().getId()));
    ...
    public void processOperatorMessage(ChannelMessage message, Stage stage) {
        if (message.source() == ChannelMessageSource.CHAT) {
            stage.getData(NoneHandler.TICKET_ID_STATE_DATA)
                    .map(Long::parseLong)
                    .flatMap(ticketService::findById)
                    .ifPresent(ticket -> replyToUser(message, ticket));
        } else {
        ...
        if (ticket.getStatus() == TicketStatus.RESOLVED) {
            ...
            for (var hub : hubService.findHubsByChannelId(handlingConfiguration.getRouteChannels(message.channelId()))) {
                ...
                if (hub.getType() == HubType.GROUP) {
                    ...
                    stageRepository.findByChannelTypeAndUserIdAndChatId(hub.getChannelId(), hub.getChannelHubId())
                            .ifPresent(stage -> {
                                if (stage.getData(NoneHandler.TICKET_ID_STATE_DATA).isPresent()) {
                                    stage.removeData(NoneHandler.TICKET_ID_STATE_DATA);
                                    stageRepository.updateByChannelTypeAndUserIdAndChatId(hub.getChannelId(), hub.getChannelHubId(), stage);
                                }
                            });
                    // Отправляем сообщение оператору
